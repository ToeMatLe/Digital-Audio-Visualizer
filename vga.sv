module vga
    ...
    (
        input  logic clk,
        input  logic rst,
        input  logic [9:0] bar_heights [0:BARS-1],
        output logic hsync,
        output logic vsync,
    ...
    localparam BAR_WIDTH = HPIXELS / BARS;
    localparam BAR_INDEX_WIDTH = (BARS <= 1) ? 1 : $clog2(BARS);
    ...
    always_comb begin
        bar_index_calc = int'(hc) / BAR_WIDTH;
        h_remainder = int'(hc) % BAR_WIDTH;

        if (bar_index_calc >= BARS)
            bar_index_calc = BARS - 1;

        bar_index = bar_index_calc[BAR_INDEX_WIDTH-1:0];
        current_height = bar_heights[bar_index];
        within_bar = (h_remainder < (BAR_WIDTH - 2));
